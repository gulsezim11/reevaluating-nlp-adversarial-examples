...
def parse_args():
    ...
    parser.add_argument('--num_train_epochs', '--epochs', type=int,
                        default=10, help='Total number of epochs to train for')
...
def main():
    ...
    if (args.augmentation_recipe is not None) and len(args.augmentation_recipe):
        ...
        if args.augmentation_recipe == 'textfooler':
            aug_csv = '/home/users/gduisen/section_6_adjusted_attacks/recipes/adversarial_results.csv'
        ...
        aug_df = pd.read_csv(aug_csv, delimiter=';')
        aug_df = aug_df.loc[:, ~aug_df.columns.str.contains('^Unnamed')]
        print(aug_df)
        ...
        train_text = [text.lower().strip() for text in train_text]
        original_text = [text.lower().strip().replace('[[', '').replace(']]', '') for text in original_text]
        ...
        for x, x_adv in zip(original_text, perturbed_text):
            matching_indices = [i for i, t in enumerate(train_text) if t == x]
            if not matching_indices:
                logger.warning(f"No match found for original_text: {x}")
                continue
            x_idx = matching_indices[0]
    ...
    for epoch in tqdm.trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        prog_bar.set_description(f"Accuracy of epoch {eval_acc}")
    ...
    plt.title('Epoch-wise Validation Accuracy', fontsize=16)
    ...
    plt.savefig(os.path.join(args.output_dir, 'accuracy_plot_textfooler.png'))
    plt.show()
